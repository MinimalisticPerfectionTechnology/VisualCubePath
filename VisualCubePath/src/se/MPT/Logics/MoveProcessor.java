...
//			ui.setFinalAnswere("[Solved]");
	...
	private Move[] generateAnswere() {
			Move[] fromSolvedArray = new Move[0];
			Move[] fromShuffledArray = new Move[0];
			if(result != null) {
				fromShuffledArray = result.getMoves();
			}
			if(result2 != null) {
				fromSolvedArray = Utils.reverseMoves(result2.getMoves());
			}
			Move[] both = Stream.concat(Arrays.stream(result.getMoves()), Arrays.stream(Utils.reverseMoves(result2.getMoves())))
                    .toArray(Move[]::new);
			return both;
	}
